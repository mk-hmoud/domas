...
export class AccessCardsRepository {
  ...
  async createBatch(
    ...
    const query = `
      INSERT INTO card_batches (location_id, name, range_start, range_end, created_by)
      VALUES ($1, $2, $3, $4, $5)
      RETURNING id, location_id as "locationId", name, range_start as "rangeStart", 
                range_end as "rangeEnd", created_at as "createdAt", 
  ...
  async findAllBatches(): Promise<CardBatch[]> {
    const query = `
      SELECT id, location_id as "locationId", name, range_start as "rangeStart", 
             range_end as "rangeEnd", created_at as "createdAt", 
             updated_at as "updatedAt", created_by as "createdBy"
      FROM card_batches
  ...
  async assignRandomCard(
    ...
    const query = `
      ...
      RETURNING id, batch_id as "batchId", card_number as "cardNumber", status,
  ...
  async findAvailableCard(batchId?: number, client?: PoolClient): Promise<AccessCard | null> {
    let query = `
      SELECT id, batch_id as "batchId", card_number as "cardNumber", status,
             current_holder_id as "currentHolderId", current_booking_id as "currentBookingId",
             issued_at as "issuedAt", issued_by as "issuedBy", returned_at as "returnedAt",
             created_at as "createdAt", updated_at as "updatedAt"
      FROM access_cards
  ...
  async findByCardNumber(cardNumber: number, client?: PoolClient): Promise<AccessCard | null> {
    const query = `
      ...
      WHERE card_number = $1
    `;
    const result = await this.getClient(client).query(query, [cardNumber]);
    return result.rows[0] ? new AccessCard(result.rows[0]) : null;
  }
